...
int main(int argc, char **argv)
{
    ...
    char tempa[2000],tempc[2000],syscmd[2000],file_absence[2000],file_pavseq[2000],file_pavout[2000],workdir[2000];
    char file_read1[900],file_read2[900],samname[500],file_confirm[500];
    ...
    if(argc < 2)
    {
         printf("Usage: %s -nodes 30 <absence_assembly_fasta> <PAV.fasta> <input_read_1>> <input_read_2> <output_PAV_file>\n",argv[0]);
...
//    system (syscmd);
    ...
    memset(file_pavseq,'\0',2000);
    sprintf(file_absence,"%s/%s",tempa,argv[args]);
    sprintf(file_pavseq,"%s/%s",tempa,argv[args+1]);
    sprintf(file_read1,"%s/%s",tempa,argv[args+2]);
    sprintf(file_read2,"%s/%s",tempa,argv[args+3]);
    sprintf(file_confirm,"%s/%s",tempa,argv[args+4]);

    memset(syscmd,'\0',2000);
    sprintf(syscmd,"%s/scanPAV_fasta -name Extract -len 10 %s PAVseq.fasta PAVseq.tag > try.out",bindir,file_pavseq);
    if(system(syscmd) == -1)
    {
//      printf("System command error:\n);
    }
...
//      printf("System command error:\n);
    ...
    memset(syscmd,'\0',2000);
    sprintf(syscmd,"cat absence.fasta PAVseq.fasta > absence-plus.fasta",bindir);
    if(system(syscmd) == -1)
    {
//      printf("System command error:\n);
    }
    ...
    memset(syscmd,'\0',2000);
    sprintf(syscmd,"%s/bwa index absence-plus.fasta > try.out",bindir);
    if(system(syscmd) == -1)
    {
//      printf("System command error:\n);
    }
    ...
    memset(syscmd,'\0',2000);
    sprintf(syscmd,"%s/bwa mem -t %d absence-plus.fasta %s %s > align.sam",bindir,n_nodes,file_read1,file_read2);
    if(system(syscmd) == -1)
    {
//    printf("System command error:\n);
    }
    ...
    memset(syscmd,'\0',2000);
    sprintf(syscmd,"cat align.sam | egrep 'Extract' |  awk '%s' > align.dat","($2<100)&&($5>0){print $1,$2,$3,$4,$5}");
    printf("%s\n",syscmd);
    if(system(syscmd) == -1)
    {
//      printf("System command error:\n);
    ...
    memset(syscmd,'\0',2000);
    sprintf(syscmd,"%s/scaff_output tarseq.fastq contig.dat genome.fastq > try.out",bindir);
    if(system(syscmd) == -1)
    {
//      printf("System command error:\n);
    }
...
//      printf("System command error:\n);
    ...
    memset(syscmd,'\0',2000);
    sprintf(syscmd,"mv genome.fasta %s",file_confirm);
    if(system(syscmd) == -1)
    {
//      printf("System command error:\n);
    }
