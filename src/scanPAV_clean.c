...
static int *insert,*hit_score,*hit_score,*ctg_list,*ctg_locist,*ctg_locied,*ctg_mask;
...
int main(int argc, char **argv)
{
    ...
    char line[2000]={0},tempc1[60],rdname[60],score[3];
...
/*
    ...
    nseq = nseq+1000;
    if((ctg_mask= (int *)calloc(nseq,sizeof(int))) == NULL)
    {
...
/*
    ...
    if((ctg_locist= (int *)calloc(nseq,sizeof(int))) == NULL)
    {
      printf("ERROR Memory_Allocate: Align_Process - ctg_head\n");
      exit(1);
    }
    if((ctg_locied= (int *)calloc(nseq,sizeof(int))) == NULL)
    {
...
/*  read the alignment files         */
    i=0;
//    while(fscanf(namef,"%s %d %s",tempc1,&hit_score[i],R_Name[i])!=EOF)
    while(fscanf(namef,"%s %s %s %s %s %s %s %s %s %s %s %s %s",rdname,tempc1,S_Name[i],R_Name[i],tempc1,tempc1,tempc1,tempc1,tempc1,tempc1,tempc1,tempc1,tempc1)!=EOF)
    {
        score[0] = rdname[12];
        score[1] = rdname[13];
        hit_score[i] = atoi(score);
        i++;
    }
    fclose(namef);
...
/* =============================== */
{
     ...
     for(i=2;i<nSeq;i++)
     {
        if((R_Name[i][0] == '*')&&(R_Name[i-1][0] != '*')&&(R_Name[i-2][0] == '*'))
        {
                ctg_mask[i-1] = 1;
//                printf("xxx: %s %s %s %s %d\n",S_Name[i-1],R_Name[i-2],R_Name[i-1],R_Name[i],hit_score[i-1]);
        } 
     }
