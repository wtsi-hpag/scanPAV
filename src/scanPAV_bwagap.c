...
static int *insert,*hit_prectg,*hit_rcdex,*hit_locus1,*hit_locus2,*readlength,*superlength;
...
static int *hit_prelos,*indel2copy,*indel2ctgs,*insert_size;
...
int main(int argc, char **argv)
{
    ...
    void Presence_Process(char **argv,int args,int nSeq);
    void Memory_Allocate(int arr);
    char line[2000]={0},tempc1[60],tmp[60],RC[2],*st,*ed;
    ...
    if(argc < 2)
    {
      printf("Usage: %s <input_align file> <output_align file>\n",argv[0]);
...
/*
    ...
    if((hit_prelos = (int *)calloc(nseq,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - hit_prelos\n");
      exit(1);
    }
    if((hit_prectg = (int *)calloc(nseq,sizeof(int))) == NULL)
    {
...
/*  read the alignment files         */
    i=0;
    while(fscanf(namef,"%s %s %s %s %d %d %d %d %s %d %s %d %d",tempc1,tempc1,R_Name[i],S_Name[i],&hit_read1[i],&hit_read2[i],&hit_locus1[i],&hit_locus2[i],RC,&hit_length[i],tempc1,&readlength[i],&superlength[i])!=EOF)
    {
        if(RC[0] == 'F')
          hit_rcdex[i]=0;
        else
	...
	strncpy(T_Name[i],R_Name[i],ed-st);
        memset(tmp,'\n',60);
	strcpy(tmp,ed+2);
        hit_prelos[i] = atoi(tmp);
        memset(tmp,'\n',60);
        ed = strchr(R_Name[i],'_');
	strncpy(tmp,ed+1,8);
	hit_prectg[i] = atoi(tmp);
//        printf("www: %s %d %d\n",R_Name[i],hit_prectg[i],hit_prelos[i]);
        i++;
    }
    fclose(namef);
...
//    Readname_match(seq,argv,args,n_reads,nRead);
    Presence_Process(argv,args,n_reads);
...
/* =============================== */
{
     ...
     for(i=0;i<(nSeq-1);i++)
     {
        ...
        {
          int gap_size;
//          printf("www: %s %d %s\n",R_Name[i],hit_prelos[i],R_Name[j-1]);
          if(hit_prelos[i] > 0)
          {
            m = hit_prelos[i]/1000;
            for(k=0;k<m;k++)
            {
               int idd = k*1000;
               fprintf(namef2,"alignment:N:00 0 %s_X%08d *        0        0         0         0   *       0  0.00 1000 0\n",T_Name[i],idd);
            }
          }
	  for(n=i;n<(j-1);n++)
	  {
             gap_size = hit_prelos[n+1]-hit_prelos[n];
             m = gap_size/1000;
             fprintf(namef2,"alignment:S:60 1000 %s %s %d %d %d %d %d 1000 99.80 1000 %d\n",R_Name[n],S_Name[n],hit_read1[n],hit_read2[n],hit_locus1[n],hit_locus2[n],hit_rcdex[n],superlength[n]);
             for(k=1;k<m;k++)
             {
                int idd = hit_prelos[n]+k*1000;
                fprintf(namef2,"alignment:N:00 0 %s_X%08d *        0        0         0         0   *       0  0.00 1000 0\n",T_Name[n],idd);
             }
          }
        }
