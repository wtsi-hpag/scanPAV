...
static int *ctg_list,*ctg_head,*ctg_length,*ctg_locus1,*ctg_locus2,*ctg_index;
...
{
    FILE *namef;
    int i,nSeq,args,nRead,nseq;
    char line[2000]={0},tempc[60],tempc1[60],rdname[60];
    char **cmatrix(long nrl,long nrh,long ncl,long nch);
    void Locus_Process(char **argv,int args,int nSeq);
...
/*
    ...
    if((ctg_locus1 = (int *)calloc(nseq,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - ctg_locus1\n");
      exit(1);
    }
    if((ctg_locus2 = (int *)calloc(nseq,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - ctg_locus2\n");
      exit(1);
    }
...
/*  read the alignment files         */
    i=0;
    while(fscanf(namef,"%s %s %d %d %d %d",R_Name[i],S_Name[i],&ctg_length[i],&ctg_index[i],&ctg_locus1[i],&ctg_locus2[i])!=EOF)
    {
        i++;
    }
    fclose(namef);

    Locus_Process(argv,args,nSeq);
...
/* =============================== */
{
     int i,j,k,m,n,n_blocks;
     FILE *namef;
     int num_hits,num_hit,num_hit2;
     ...
     for(i=0;i<nSeq;i++)
     {
        ...
        j=i+1;
        ctg_list[n_blocks]++;
        while((j<nSeq)&&(stopflag==0)&&(strcmp(R_Name[i],"gapsm:") == 0))
        {
          if((j<nSeq)&&(strcmp(R_Name[j],"match:") == 0)) 
          {
            ctg_list[n_blocks]++;
            j++;
          }
        ...
        num_hit = j-i;
//      printf("map2: %d %s %d %d\n",n_blocks,R_Name[i],num_hit,ctg_list[n_blocks]);
        for(k=i;k<j;k++) 
        {
//    printf("%s %s %d %d %d %d %d\n",R_Name[k],S_Name[k],ctg_length[k],ctg_index[k],ctg_locus1[k],ctg_locus2[k],ctg_list[n_blocks]);
        }
        i=j-1;
        n_blocks++;
     }
     ...
     {
        k = ctg_head[i];
//    printf("%s %s %d %d %d %d\n",R_Name[k],S_Name[k],ctg_length[k],ctg_index[k],ctg_locus1[k],ctg_locus2[k]);
        for(j=1;j<ctg_list[i];j++)
        {
           int idd = ctg_head[i]; 
           int idt = ctg_head[i]+j;
           int out_flag = 0;
           int idi;
           for(m=(i+1);m<n_blocks;m++)
           {
              n = ctg_head[m];
...
//                printf("match-ins: %s %d %d %d %d %d %d\n",S_Name[n],ctg_length[n]/1000,ctg_index[n],ctg_locus1[n],ctg_locus1[n]+ctg_length[n],ctg_locus2[n],ctg_locus2[n]+ctg_length[n]);
           ...
           if(out_flag==0)
           {
//             printf("%s %s %d %d %d %d %d %d\n",R_Name[idt],S_Name[idt],ctg_length[idt],ctg_index[idt],ctg_locus1[idt],ctg_locus2[idt],ctg_locus1[idt]-ctg_locus1[idd]+ctg_locus2[idd],ctg_locus2[idt]-ctg_locus1[idd]+ctg_locus2[idd]);
             fprintf(namef,"%s %s %d %d %d %d %d %d\n",R_Name[idt],S_Name[idt],ctg_length[idt],ctg_index[idt],ctg_locus1[idt],ctg_locus2[idt],ctg_locus1[idt]-ctg_locus1[idd]+ctg_locus2[idd],ctg_locus2[idt]-ctg_locus1[idd]+ctg_locus2[idd]);
           }    
//    printf("%s %s %d %d %d %d %d %d %d\n",R_Name[idt],S_Name[idt],ctg_length[idt],ctg_index[idt],ctg_locus1[idt],ctg_locus2[idt],ctg_list[i],ctg_locus1[idt]-ctg_locus1[idd]+ctg_locus2[idd],ctg_locus2[idt]-ctg_locus1[idd]+ctg_locus2[idd]);
        }
//    printf("\n");
     }
     fclose(namef);
